...
import { Zap, Star, ArrowRight, ShieldCheck, CheckSquare, QrCode, CheckCircle2, X, Loader2, Lock, Check } from "lucide-react";
...
const PLANS = [
  {
    ...
    icon: ShieldCheck,
    monthlyPrice: 799,
    annualPrice: 7990, // Approx 2 months free
    limit: "100 Students",
  ...
  },
  {
    ...
    features: [
      ...
      "Automated Fee Enquiries",
      "Marketing Website",
      "CRM with Custom Domain",
...
export default function UpgradeSection({ isPaid, daysLeft, currentPlan }: Props) {
  ...
  // Determine current user's tier to handle lock/unlock logic
  const currentTierObj = PLANS.find(p => p.name === currentPlan);
  const currentTierLevel = isPaid && currentTierObj ? currentTierObj.tier : 0; // 0 = Free Trial
  ...
  const handleSubmitPayment = (e: React.FormEvent) => {
    ...
    // Simulate API call to save UTR receipt
    setTimeout(() => {
  ...
  // Generate Real Scannable UPI QR Code
  const getDynamicQR = (price: number) => {
    const upiId = "7080626215@ybl";
    const payeeName = "CoachingWala";
    // Standard UPI intent string
    const upiString = `upi://pay?pa=${upiId}&pn=${payeeName}&am=${price}&cu=INR`;
  ...
  return (
    <div className="space-y-6">
            ...
            {paymentState === "success" ? (
              <div className="p-8 text-center">
                ...
                <p className="text-sm font-medium text-slate-500 mb-6">
                  We have received your UTR: <span className="font-bold text-slate-800">{utr}</span>. 
                  Your <b>{checkoutPlan.name}</b> features will be unlocked as soon as our team verifies the transaction (usually within 1-2 hours).
                </p>
              ...
              <div className="p-6 sm:p-8">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <p className="text-sm text-slate-500 font-medium mt-1">Scan & pay via any UPI app (GPay, PhonePe, Paytm).</p>
                  </div>
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 mb-6 text-center shadow-inner">
                  ...
                  <p className="text-xs text-slate-500 mt-1 font-semibold">{billingCycle === "monthly" ? "for 1 month" : "for 1 year (discounted)"}</p>
                </div>
                ...
                <div className="flex flex-col items-center mb-6">
                  ...
                  <div className="w-40 h-40 bg-white border-2 border-[#0055a5]/20 p-2 rounded-2xl flex flex-col items-center justify-center mb-3 shadow-md">
                    <img 
                      src={getDynamicQR(billingCycle === "monthly" ? checkoutPlan.monthlyPrice : checkoutPlan.annualPrice)} 
                      alt="UPI QR Code" 
                ...
                <form onSubmit={handleSubmitPayment}>
                  ...
                  />
                  <button 
                    disabled={paymentState === "processing" || utr.length < 6}
                    type="submit" 
                    className="w-full flex items-center justify-center gap-2 bg-[#0055a5] hover:bg-[#004080] disabled:opacity-50 text-white font-bold py-4 rounded-xl shadow-lg shadow-[#0055a5]/20 transition-all active:scale-[0.98]"
                  >
                    {paymentState === "processing" ? <Loader2 className="w-5 h-5 animate-spin" /> : "Verify Payment"}
                  </button>
                </form>
              </div>
            )}
      ...
      {/* ── TOP BANNER (Adapts if Paid or Trial) ── */}
      <div className="flex flex-col md:flex-row items-center justify-between gap-5 bg-gradient-to-r from-[#003366] via-[#004b87] to-[#0066cc] rounded-3xl px-8 py-7 shadow-inner backdrop-blur-sm">
        ...
        <div className="flex items-center bg-[#002244]/50 p-1.5 rounded-xl border border-white/10 shrink-0">
          <button 
            ...
            className={`px-5 py-2.5 rounded-lg text-sm font-black transition-all ${billingCycle === "monthly" ? "bg-white text-[#003366] shadow-sm" : "text-white/60 hover:text-white"}`}
          >
            Monthly
          </button>
          <button 
            onClick={() => setBillingCycle("annual")}
            className={`px-5 py-2.5 rounded-lg text-sm font-black transition-all flex items-center gap-2 ${billingCycle === "annual" ? "bg-white text-[#003366] shadow-sm" : "text-white/60 hover:text-white"}`}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {PLANS.map(plan => {
            ...
            return (
              <div
                key={plan.key}
                className={`border rounded-2xl p-6 flex flex-col items-start relative overflow-hidden transition-all ${
                  isThisPlan 
                    ? "border-emerald-500 bg-emerald-50/30 shadow-lg ring-2 ring-emerald-500/10 scale-100 lg:scale-[1.02] z-10"
                    : plan.name === "Growth Plan" && !isPaid
                    ? "border-[#0055a5] bg-blue-50/30 shadow-lg ring-2 ring-[#0055a5]/10 scale-100 lg:scale-[1.02] z-10"
                    : "border-slate-200 bg-white hover:border-[#0055a5]/30 hover:shadow-md"
                }`}
              >
                {/* Top Highlight Bars */}
                ...
                <div className="flex items-center gap-3.5 mb-4 w-full justify-between mt-1">
                  ...
                  <div className="text-right">
                    <p className={`text-[11px] font-black px-2.5 py-1 rounded-md ${isThisPlan ? "bg-emerald-100 text-emerald-700" : "bg-blue-100 text-[#0055a5]"}`}>
                      LIMIT: {plan.limit}
                    </p>
                  </div>
                </div>
                ...
                {/* Special Offer Label */}
                {billingCycle === "annual" && plan.name === "Growth Plan" && (
                  <p className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded inline-block mb-4">
                    🎉 Special 15k offer applied
                  </p>
                )}
                {/* Spacing alignment */}
                ...
                <div className="space-y-3.5 mb-8 w-full">
                  {plan.features.map((f, i) => (
                    <div key={i} className="flex items-start gap-3">
                      {isIncluded || isThisPlan ? (
                        <CheckSquare className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5"/>
                      ) : (
                        <CheckSquare className="w-4 h-4 text-[#0055a5] shrink-0 mt-0.5"/>
                      )}
                ...
                <div className="mt-auto w-full">
                  {isThisPlan ? (
                    <button disabled className="w-full flex items-center justify-center gap-2 bg-emerald-500 text-white text-sm font-black px-4 py-3.5 rounded-xl shadow-md border border-emerald-600">
                      <CheckCircle2 className="w-4 h-4" /> Active Plan
                    </button>
                  ) : isIncluded ? (
                    <button disabled className="w-full text-center bg-slate-50 text-slate-500 text-sm font-black px-4 py-3.5 rounded-xl border border-slate-200">
                      Included in your Plan
                    </button>
                  ) : (
                    <button 
                      onClick={() => setCheckoutPlan(plan)}
                      className={`w-full flex items-center justify-center gap-2 text-sm font-black px-4 py-3.5 rounded-xl transition-all shadow-sm active:scale-[0.98] ${
                        plan.name === "Growth Plan" 
                          ? "bg-[#0055a5] hover:bg-[#004080] text-white shadow-md shadow-[#0055a5]/20" 
                          : "bg-white border-2 border-slate-200 text-slate-800 hover:border-[#0055a5] hover:text-[#0055a5]"
                      }`}
                    >
