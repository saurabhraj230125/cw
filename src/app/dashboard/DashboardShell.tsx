...
import { 
  ...
  Layers, BookMarked, Lock, Loader2, ChevronRight
} from "lucide-react";
...
const navigation = [
  ...
  // 🚨 LOCKED PREMIUM FEATURES (Routes directly to Settings/Billing)
  { name: "DPP & Study Material", href: "/dashboard/settings", icon: BookOpen, locked: true },
  { name: "Online Tests", href: "/dashboard/settings", icon: CheckSquare, locked: true },
  { name: "Analytics & Reports", href: "/dashboard/settings", icon: BarChart3, locked: true },
  { name: "System Alerts", href: "/dashboard/settings", icon: BellRing, locked: true },
];
    ...
    <div className="min-h-screen bg-[#f3f4f6] flex flex-col font-sans overflow-hidden selection:bg-[#0055a5] selection:text-white">
      ...
      <header className="h-[70px] bg-gradient-to-r from-[#003366] via-[#004b87] to-[#0066cc] flex items-center justify-between px-4 sm:px-6 shrink-0 z-30 shadow-md">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <Link href="/dashboard/settings" className="h-[38px] w-[38px] sm:h-[42px] sm:w-[42px] rounded-full bg-slate-200 border border-white/20 overflow-hidden shadow-sm shrink-0 flex items-center justify-center cursor-pointer hover:ring-2 hover:ring-white/50 transition-all">
            <User className="w-5 h-5 sm:w-6 sm:h-6 text-slate-500 mt-1.5 sm:mt-2" />
          </Link>

          {/* Working Red Power Button with Loader */}
          <button 
            onClick={handleLogout}
            disabled={isLoggingOut}
            title="Secure Logout"
            className="h-[38px] w-[38px] sm:h-[44px] sm:w-[44px] rounded-full bg-gradient-to-b from-[#ff4d4d] to-[#cc0000] border-2 border-[#990000] shadow-[inset_0_2px_4px_rgba(255,255,255,0.4),_0_2px_5px_rgba(0,0,0,0.5)] flex items-center justify-center hover:from-[#ff6666] hover:to-[#e60000] active:shadow-[inset_0_3px_5px_rgba(0,0,0,0.6)] transition-all shrink-0 cursor-pointer disabled:opacity-50"
          >
            {isLoggingOut ? (
          ...
          <nav className="flex-1 overflow-y-auto pb-6 pt-2 hide-scrollbar">
            ...
            {navigation.map((item) => {
              const isActive = item.href === '/dashboard' 
                ? pathname === '/dashboard'
                : pathname.startsWith(item.href) && !item.locked; // Locked items shouldn't highlight as active if they share the settings route
              ...
              return (
                <Link
                  ...
                  } ${isLocked ? "bg-slate-50/50 opacity-90" : ""}`}
                >
                  ...
                  {/* Small UI Lock for Premium Features */}
                  {isLocked && (
                    <span title="Premium Feature - Upgrade Required" className="flex items-center">
            ...
            {/* UPGRADED ERP TRIAL WIDGET (Now routes to Settings) */}
            <Link 
              href="/dashboard/settings" 
              onClick={handleNavClick}
              className="block mx-3 mt-6 mb-4 border border-[#0055a5]/20 bg-gradient-to-b from-[#ffffff] to-[#f8fafc] shadow-sm hover:shadow-md transition-all rounded-[6px] overflow-hidden group cursor-pointer"
            >
              <div className="p-2.5 border-b border-[#0055a5]/10 bg-white flex justify-between items-center">
                <span className="text-[#cc0000] font-bold text-[11px] uppercase tracking-wide flex items-center gap-1">
                  <Zap className="w-3 h-3 fill-[#cc0000]" /> Trial Active
                </span>
                <span className="text-[#0055a5] font-extrabold text-[11px]">7 Days Left</span>
              </div>
              
              <div className="p-3">
                <div className="w-full bg-[#e2e8f0] h-1.5 mb-3 rounded-full overflow-hidden border border-gray-200">
                  <div className="bg-[#008000] h-full w-[20%]"></div>
                </div>
                
                <div className="flex items-center justify-center gap-1.5 w-full bg-[#0055a5] group-hover:bg-[#004080] text-white py-2 text-[12px] font-bold transition-colors rounded-[4px] shadow-sm">
